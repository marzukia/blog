...
seotitle: "qMLX: Minmaxing my Mac Studio"
date: "2026-07-10"
...
*   **qMLX** ([marzukia/qMLX](https://github.com/marzukia/qMLX)): the fork. Disk KV checkpoint and restore, the matchable-aware eviction from bug three, hybrid-cache-aware accounting, and the phase-split metrics surface.
*   **The benchmark tool** (`bench_qmlx.py`, in the repo): the pp/tg sweep behind the charts above. It hits the running server with unique prompts so the cache cannot fake the prefill number, phase-splits prefill and decode, and has a compare mode that runs each context size cold then warm to show exactly what the cache does. Rerun it against your own box.
